...
while True:
    ...
    # Set auto_adjust=False to ensure we get standard OHLC columns (Open, High, Low, Close)
    data = yf.download(stock_ticker, start=one_year_ago, end=today, auto_adjust=False)
    print(data)
    if data.empty:
        ...
        y = last_year['Close'].values.flatten()  # Convert to 1D array to fix sklearn warning
        ...
        # For predicted rows, estimate High and Low based on Close price
        # Use a simple approach: High = Close * 1.02, Low = Close * 0.98
        # This is a reasonable approximation for daily volatility
        # Check if 'High' column exists, if not create it
        if 'High' not in predictions_df.columns:
            predictions_df['High'] = np.nan
        if 'Low' not in predictions_df.columns:
            predictions_df['Low'] = np.nan
        
        mask = predictions_df['High'].isna()
        predictions_df.loc[mask, 'High'] = predictions_df.loc[mask, 'Close'] * 1.02
        predictions_df.loc[mask, 'Low'] = predictions_df.loc[mask, 'Close'] * 0.98
